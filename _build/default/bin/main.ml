...
let player_loop map (players : player array) : map * player array =
  let rec handler map players =
    print_endline
      ...
      \ \"map\" to see the map, \n\
      \ \"player [player name]\" to check information about [player \
       name],\n\
          ...
          | _ -> (
              print_endline
               ...
               ^ "\n\
                 ...
                 \ \"sell [grid index]\" to sell your land at [grid \
                  index],");
              (ANSITerminal.print_string [ ANSITerminal.red ])
                " (ATTENTION: if your balance can't afford the toll, \
                 your land properties will be sold at 50% of the \
                 original price; so consider selling if you need more \
                 money.) \n\
                \ ";
              print_endline
                "\"upgrade\" to upgrade the land you are standing on \
